...
int StringTokenizer::countTokens() {
	...
	if (token_str.length() > 0) {
		...
		unsigned long long curr_pos = 0;
		while (true) {
			if ((curr_pos = token_str.find(delim, curr_pos))
					!= std::string::npos) {
				num_tokens++;
				prev_pos = curr_pos;
				curr_pos += delim.length();
...
std::string StringTokenizer::nextToken() {
	...
	std::string tmp_str = "";
	unsigned long long pos = token_str.find(delim, 0);

	if (pos != std::string::npos) {
		tmp_str = token_str.substr(0, pos);
		token_str = token_str.substr(pos + delim.length(),
				token_str.length() - pos);
	} else {
...
long StringTokenizer::nextLongToken() {
...
std::string StringTokenizer::nextToken(const std::string& delimiter) {
	...
	std::string tmp_str = "";
	unsigned long long pos = token_str.find(delimiter, 0);

	if (pos != std::string::npos) {
		tmp_str = token_str.substr(0, pos);
		token_str = token_str.substr(pos + delimiter.length(),
				token_str.length() - pos);
	} else {
		tmp_str = token_str.substr(0, token_str.length());
		token_str = "";
